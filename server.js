...
//Add some set up datas
app.get('/admin/initdb', (req, res) => {
  db.serialize(function() {
    db.run("CREATE TABLE mytable (id INTEGER PRIMARY KEY, name TEXT)");
    db.run("CREATE TABLE PROFILE (id INT PRIMARY KEY,name VARCHAR(255) NOT NULL,description TEXT NOT NULL);")
    db.run("CREATE TABLE SKILL (id INT PRIMARY KEY, name VARCHAR(255) NOT NULL, description TEXT NOT NULL);")
    db.run("CREATE TABLE PROFILE_SKILL (profile_id INT NOT NULL, skill_id INT NOT NULL, PRIMARY KEY (profile_id, skill_id), FOREIGN KEY (profile_id) REFERENCES PROFILE(id), FOREIGN KEY (skill_id) REFERENCES SKILL(id));")
    db.run("INSERT INTO PROFILE (id, name, description) \
      VALUES\
    ...
    db.run("INSERT INTO SKILL (id, name, description)\
      VALUES\
    ...
    db.run("INSERT INTO PROFILE_SKILL (profile_id, skill_id)\
    VALUES\
